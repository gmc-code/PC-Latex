...
\def\angleIncidence{60}
\def\angleRefraction{36}
% Angle labels using \edef to embed values
\edef\incLabel{$i=\angleIncidence^\circ$}
\newcommand{\refLabel}{\ensuremath{r=\raisebox{-2.2ex}{\text{\Large$  \cdot\ \cdot\ \cdot$}}^\circ}}
% Description variable
\def\refractionDescription{Light travelling from air into an acrylic block bends \raisebox{-2.2ex}{\Large\hbox to 4cm{\leaders\hbox{$\cdot$\hskip 0.6em}\hfill}} the normal.}
...
\begin{tikzpicture}[remember picture,overlay]
  ...
  % Layer 2: Rectangle and rays
    ...
    \draw[dashed, line width=0.5pt]
      ($(topCenter)+(0,-2cm)$) -- ($(topCenter)+(0,2cm)$);

    % Incident ray
    \coordinate (incidentEnd) at ($(topCenter)+( {90 + \angleIncidence} :4cm)$);
    \draw[gray, postaction={decorate},
      decoration={markings, mark=at position 0.8 with {\arrowreversed[scale=1.5]{Latex}}},dash pattern=on 1.2cm off 1.8cm]
      (topCenter) -- (incidentEnd);

    % Refracted ray
    \coordinate (refractEnd) at ($(topCenter)+( { \angleRefraction - 90 } :4cm)$);
    \draw[-{Latex[length=3mm]}, gray,dash pattern=on 1.2cm off 1.8cm]
    ...
    \pic [draw, -, angle radius=10mm]
      ...
      \node at ($(topCenter)+ (18mm, 8mm)$) {\LARGE \incLabel};

    % Angle of refraction
    \pic [draw, -, angle radius=10mm]
      {angle = normalDown--topCenter--refractEnd};
    \node at ($(topCenter)+ (20mm, -8mm)$) {\LARGE \refLabel};
