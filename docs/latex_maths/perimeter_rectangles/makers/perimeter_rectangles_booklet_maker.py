...
tex_template_path = currfile_dir / "perimeter_rectangles_booklet_template.tex"
texans_template_path = currfile_dir / "perimeter_rectangles_booklet_ans_template.tex"
tex_diagram_template_path = currfile_dir / "perimeter_rectangles_booklet_diagram_template.tex"
...
def make1_diagram(tex_diagram_template_txt, side_pairs, rotation):
    ...
    kv = arf.get_perimeter_rectangles_dict(side_pairs, rotation)
...
def main():
    ...
    filename = input(
        "Enter the base filename to be added to the prefix perimeter_rectangles_Bk_: \n"
    )
    if not filename:
        filename = "1"  # "perimeter_rectangles_Bk_1_q and perimeter_rectangles_Bk_1_ans as default file"
    # set names of files that are made
    # questions
    tex_output_path = currfile_dir / f"perimeter_rectangles_Bk_{filename}_q.tex"
    pdf_path = currfile_dir / f"perimeter_rectangles_Bk_{filename}_q.pdf"

    # answers
    tex_output_path_ans = currfile_dir / f"perimeter_rectangles_Bk_{filename}_ans.tex"
    pdf_path_ans = currfile_dir / f"perimeter_rectangles_Bk_{filename}_ans.pdf"
