...
tex_template_path = currfile_dir / "like_terms_template.tex"
texans_template_path = currfile_dir / "like_terms_template.tex"
tex_diagram_template_path = currfile_dir / "like_terms_diagram_template.tex"
...
def make1_diagram(tex_diagram_template_txt, num):
    ...
    kv = ltf.get_1step_process_dict(num)
...
def main():
    ...
    if num.strip().isdigit():
        ...
        num = 5  # random by default
    filename = input("Enter the base filename to be added to the prefix like_terms1_: \n")
    if not filename:
        filename = "1"  # "like_terms1_1_q and like_terms1_1_ans as default file"
    # set names of files that are made
    # questions
    tex_output_path = currfile_dir / f"like_terms1_{filename}_q.tex"
    pdf_path = currfile_dir / f"like_terms1_{filename}_q.pdf"
    png_path = currfile_dir / f"like_terms1_{filename}_q.png"
    # answers
    tex_output_path_ans = currfile_dir / f"like_terms1_{filename}_ans.tex"
    pdf_path_ans = currfile_dir / f"like_terms1_{filename}_ans.pdf"
    png_path_ans = currfile_dir / f"like_terms1_{filename}_ans.png"
