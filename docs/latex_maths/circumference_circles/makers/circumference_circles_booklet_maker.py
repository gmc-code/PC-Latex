...
import circumference_circles_functions as ccf

currfile_dir = Path(__file__).parent
tex_template_path = currfile_dir / "circumference_circles_booklet_template.tex"
texans_template_path = currfile_dir / "circumference_circles_booklet_ans_template.tex"
tex_diagram_template_path = currfile_dir / "circumference_circles_booklet_diagram_template.tex"

# list of sides so not repeated in max of 20
side_ints_list = ccf.get_random_float_1dp_list()
rotations_list = ccf.get_rotations_shuffled()
...
# calc_radius_value, calc_circumference_value
...
def make1_diagram(tex_diagram_template_txt, side_int, rotation):
    ...
    kv = ccf.get_circumference_circles_dict(side_int, rotation)
...
def main():
    ...
    filename = input(
        "Enter the base filename to be added to the prefix circumference_circles_Bk_: \n"
    )
    if not filename:
        filename = "1"  # "circumference_circles_Bk_1_q and circumference_circles_Bk_1_ans as default file"
    # set names of files that are made
    # questions
    tex_output_path = currfile_dir / f"circumference_circles_Bk_{filename}_q.tex"
    pdf_path = currfile_dir / f"circumference_circles_Bk_{filename}_q.pdf"

    # answers
    tex_output_path_ans = currfile_dir / f"circumference_circles_Bk_{filename}_ans.tex"
    pdf_path_ans = currfile_dir / f"circumference_circles_Bk_{filename}_ans.pdf"
