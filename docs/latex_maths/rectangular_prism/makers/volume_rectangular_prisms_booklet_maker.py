...
import volume_rectangular_prisms_functions as arf

currfile_dir = Path(__file__).parent
tex_template_path = currfile_dir / "volume_rectangular_prisms_booklet_template.tex"
texans_template_path = currfile_dir / "volume_rectangular_prisms_booklet_ans_template.tex"
tex_diagram_template_path = currfile_dir / "volume_rectangular_prisms_booklet_diagram_template.tex"


# list of sides so not repeated in max of 20
side_pairs_list = arf.get_side_pairs()
rotations_list = arf.get_rotations_shuffled()
heights_list = arf.get_heights()
...
# 'calc_l_value', 'calc_w_value', 'calc_H_value', 'calc_V_value'
tex_keys_q = ['calc_l', 'calc_w', 'calc_H', 'calc_V', 'length', 'width', 'height', 'dx', 'dy', 'rotation', 'vA', 'vB', 'vC', 'vD', 'vE', 'vF', 'vG', 'vH']




def make1_diagram(tex_diagram_template_txt, side_pairs, height, rotation):
    tex_diagram_template_txt_ans = tex_diagram_template_txt
    posttext = r"\par\vspace{1cm}"  #  ~ \newline
    kv = arf.get_volume_rectangular_prisms_dict(side_pairs, height, rotation)
...
def main():
    ...
    filename = input(
        "Enter the base filename to be added to the prefix volume_rectangular_prisms_Bk_: \n"
    )
    if not filename:
        filename = "1"  # "volume_rectangular_prisms_Bk_1_q and volume_rectangular_prisms_Bk_1_ans as default file"
    # set names of files that are made
    # questions
    tex_output_path = currfile_dir / f"volume_rectangular_prisms_Bk_{filename}_q.tex"
    pdf_path = currfile_dir / f"volume_rectangular_prisms_Bk_{filename}_q.pdf"

    # answers
    tex_output_path_ans = currfile_dir / f"volume_rectangular_prisms_Bk_{filename}_ans.tex"
    pdf_path_ans = currfile_dir / f"volume_rectangular_prisms_Bk_{filename}_ans.pdf"
    ...
    for i in range(1, numq + 1):
        ...
        rotation = 0  # rotations_list[i - 1]
        height = heights_list[i - 1]


        img_tex, img_tex_ans = make1_diagram(tex_diagram_template_txt, side_pairs, height, rotation)
        if i > 4 and i % 4 == 1:
