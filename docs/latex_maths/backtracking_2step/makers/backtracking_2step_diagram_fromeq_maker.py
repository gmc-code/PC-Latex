...
def main():
    ...
    if num2.strip().isdigit():
        ...
        num2 = 5  # random by default
    filename = input("Enter the base filename to be added to the prefix bt2_fromeq_: \n")
    if not filename:
        filename = "bt2_fromeq_1st"  # "bt2_fromeq_1st_q and bt2_fromeq_1st_ans as default file"
    # set names of files that are made
    # questions
    tex_output_path = currfile_dir / f"bt2_fromeq_{filename}_q.tex"
    pdf_path = currfile_dir / f"bt2_fromeq_{filename}_q.pdf"
    png_path = currfile_dir / f"bt2_fromeq_{filename}_q.png"
    aux_path = currfile_dir / "temp"
    # answers
    tex_output_path_ans = currfile_dir / f"bt2_fromeq_{filename}_ans.tex"
    pdf_path_ans = currfile_dir / f"bt2_fromeq_{filename}_ans.pdf"
    png_path_ans = currfile_dir / f"bt2_fromeq_{filename}_ans.png"
