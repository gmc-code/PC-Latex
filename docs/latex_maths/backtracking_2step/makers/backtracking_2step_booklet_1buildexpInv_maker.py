...
def main():
    ...
    if numq.strip().isdigit():
        ...
        numq = 20  # random by default
    #
    filename = input("Enter the base filename to be added to the prefix bt2Bk_bldexpInv_: \n")
    if not filename:
        filename = "bt2Bk_bldexpInv_1st"  # "bt2Bk_bldexpInv_1st_q and bt2Bk_bldexpInv_1st_ans as default file"
    # set names of files that are made
    # questions
    tex_output_path = currfile_dir / f"bt2Bk_bldexpInv_{filename}_q.tex"
    aux_path = currfile_dir / "temp"
    # answers
    tex_output_path_ans = currfile_dir / f"bt2Bk_bldexpInv_{filename}_ans.tex"
