...
import coordinates_functions as coordf
...
kv_keys_q = ["points_to_plot_without_xy"]
ky_keys_q_to_clear = ["points_to_plot"]
kv_keys_ans = ["points_to_plot"]
ky_keys_ans_to_clear = ["points_to_plot_without_xy"]
    ...
    for key, value in kv.items():
        ...
        if key in kv_keys_ans:
            tex_diagram_template_txt_ans = tex_diagram_template_txt_ans.replace(
                "<<" + key + ">>", value)
        if key in ky_keys_ans_to_clear:
               tex_diagram_template_txt_ans = tex_diagram_template_txt_ans.replace(
                "<<" + key + ">>", "")
    for key, value in kv.items():
        # show questions
