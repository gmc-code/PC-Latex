...
import perimeter_squares_functions as psf

currfile_dir = Path(__file__).parent
tex_template_path = currfile_dir / "perimeter_squares_booklet_template.tex"
texans_template_path = currfile_dir / "perimeter_squares_booklet_ans_template.tex"
tex_diagram_template_path = currfile_dir / "perimeter_squares_booklet_diagram_template.tex"

# list of sides so not repeated in max of 20
side_ints_list = psf.get_ints_shuffled_one_dig_first()
rotations_list = psf.get_rotations_shuffled()
...
# calcside_value, calcperimeter_value
tex_keys_q = ['calc_sidelength', 'sidelength', 'rotation', 'units', 'vA', 'vB', 'vC', 'vD']


def make1_diagram(tex_diagram_template_txt, side_int, rotation):
    tex_diagram_template_txt_ans = tex_diagram_template_txt
    posttext = r"\par\vspace{1cm}"  #  ~ \newline
    kv = psf.get_perimeter_squares_dict(side_int, rotation)
...
def main():
    ...
    filename = input(
        "Enter the base filename to be added to the prefix perimeter_squares_Bk_: \n"
    )
    if not filename:
        filename = "1"  # "perimeter_squares_Bk_1_q and perimeter_squares_Bk_1_ans as default file"
    # set names of files that are made
    # questions
    tex_output_path = currfile_dir / f"perimeter_squares_Bk_{filename}_q.tex"
    pdf_path = currfile_dir / f"perimeter_squares_Bk_{filename}_q.pdf"

    # answers
    tex_output_path_ans = currfile_dir / f"perimeter_squares_Bk_{filename}_ans.tex"
    pdf_path_ans = currfile_dir / f"perimeter_squares_Bk_{filename}_ans.pdf"
